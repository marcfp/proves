...
/*	names.erase(names.end(),names.back());
	...
	int troba=-99;
//	find (names.begin(), names.end(),noms)-nom.begin();
	cout << endl << "Entra l'edat a trobar :"<< endl;
	cin >> troba;
//	it = find (scores.begin(), scores.end(), troba); //it iterator
	for(int i=0; i<scores.size();++i){
	  if (troba == scores.at(i))
	    cout << "Element found in scores at position : " << i << "\n";
	  else
	    cout << "Element NOT found in scores at position " << i << "\n";
	}
return(0);
}
