...
class Roma{
  ...
  public:
    ...
    void set_num(){
      cout << "Entra el numero en xifres romanes i et diré el seu valor en números arábics º1" << endl;
      cin >> num;
    ...
    int as_int(){ 
	  ...
	  if(to_int(num[i])==anterior && conta<=3){	    
	    if(to_int(num[i-1])<to_int(num[i]) && conta==1 )  val=val-to_int(num[i]); 
	    else if(conta<=3) val=val+to_int(num[i]);
		  else {
		    val=0;
		      cout << "número mal format!!" << endl;
		      return(0);
	...
	if(to_int(num[i-1])>val && to_int(num[0])==to_int(num[2])) {
	  cout << "número mal format" << endl;
	  return (0);
	}
      }
...
{ 
   Roma roma;
    ...
    else cout << " número mal format!!!" << endl;
